...
class TipoSolicitudController {
    ...
    createTipoSolicitud = async (req, res) => {
      try {
        ...
        const tipoSolicitud = await this.TipoSolicitudApi.createTipoSolicitud(Nombre)
        res.status(200).send({
          success: true,
    ...
    updateTipoSolicitud = async (req, res) => {
      try {
        const { Nombre } = req.body;
        const { Id } = req.params;
        const tipoSolicitud = await this.tipoSolicitudApi.updateTipoSolicitud(Nombre, Id);
        res
          .status(200)
          .send({ success: true, message: "Usuario modificado", data: tipoSolicitud });
      } catch (error) {
        res.status(400).send({ success: false, message: error.message });
      }
    };
